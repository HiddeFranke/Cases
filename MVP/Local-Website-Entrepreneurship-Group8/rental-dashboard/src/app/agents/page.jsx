...
import AgentViewerModal from '@/components/AgentViewerModal';
...
export default function AgentsPage() {
  ...
  const [viewerJobName, setViewerJobName] = useState('');
                ...
                <div key={job.id} className={`bg-white rounded-xl border ${isActive ? style.border : 'border-slate-100'} p-5 transition-all`}>
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      ...
                      {/* Watch button — always visible for running jobs */}
                      {job.status === 'running' && (
                        <button
                          onClick={() => {
                            setViewerJobName(job.propertyName || 'Agent');
                            setViewerOpen(true);
                          }}
