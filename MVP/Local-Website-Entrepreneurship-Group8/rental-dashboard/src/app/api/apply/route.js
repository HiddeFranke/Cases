...
async function applyOnPararius(property, user, aiLetter) {
  ...
  const isHeadless = isProduction ? true : (user.playwrightHeadless ?? false);
  const browser = await chromium.launch({
    headless: isHeadless,
    slowMo: isHeadless ? 100 : 350,
    args: isHeadless
...
export async function POST(request) {
  ...
  try {
    ...
    const aiLetter = await generateAILetter(property, user);
    const result = await applyOnPararius(property, user, aiLetter);
